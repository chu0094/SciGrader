...
async def proof_node(answer_unit: Dict[str, Any], rubric: str, max_score: float = 10.0, llm=None) -> Correction:
    ...
    try:
        ...
        try:
                    ...
                    try:
                        if isinstance(step, dict):
                            step_score = StepScore(
                                ...
                                desc=step.get("comment", f"Step {step.get('step_no', len(step_scores) + 1)}"),
            ...
            comment = llm_response.get("comment", f"The proof contains {len(step_scores)} steps.")
        ...
        default_prompt = f"""
        You are a mathematics teacher who needs to grade a student's answer to a proof problem.
        ...
        {{
            "overall_score": Score between 0-10,
            "max_score": 10,

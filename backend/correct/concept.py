...
async def concept_node(answer_unit: Dict[str, Any], rubric: str, max_score: float = 10.0, llm=None) -> Correction:
    """
    ...
    # Ensure answer_unit has required fields
    if "text" not in answer_unit:
        answer_unit["text"] = ""
    if "q_id" not in answer_unit:
        answer_unit["q_id"] = "unknown"
    ...
    keywords = [f"Knowledge Point {i}" for i in range(5)]  # Mock keywords
    
    # Step 2: Prepare prompt using the new prompt_utils module
    try:
        template_path = "prompts/concept.txt"
        # context = keywords
        context = "No standard knowledge points provided. Please extract knowledge points based on the problem!"
        ...
        try:
            ...
            comment = llm_response.get("comment", f"The concept problem contains {len(step_scores)} steps.")
            ...
            try:
                ...
                logger.error("correction_creation_failed", error=str(correction_error), 
                           q_id=answer_unit.get("q_id", "unknown"), type="概念题", score=total_score, 
        ...
        default_prompt = f"""
        You are a professional teacher who needs to grade a student's answer to a concept problem.

    ...
    def generate(self) -> List[TestCase]:
        ...
        return [
            TestCase(
                input="",
                expected_output="",
                description="Default test cases"
            )
        ]
...
class CodeExecutor:
    ...
    def run(self, code: str, test_cases: List[TestCase]) -> ExecutionResult:
        ...
        result = ExecutionResult(
            ...
            error="",
            logs="Execution log information",
            pass_rate=1.0,
            coverage=0.8
        )
...
async def programming_node(answer_unit: Dict[str, Any], rubric: str, max_score: float = 10.0, llm=None) -> Correction:
    ...
    steps = [
        StepScore(
            step_no=1,
            desc=f"Pass Rate: {result.pass_rate:.2%}, Coverage: {result.coverage:.2%}",
    ...
    try:
        ...
        problem = answer_unit.get("stem", "Programming problem")
        ...
        try:
            ...
            comment = llm_response.get("comment", f"Pass Rate: {result.pass_rate:.2%}, Coverage: {result.coverage:.2%}")
        ...
        default_prompt = f"""
        You are a programming teacher who needs to grade a student's answer to a programming problem.
